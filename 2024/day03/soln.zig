...
const DATA_FILE = ( // zig fmt: off
    // "D:/Files/advent/2024/day03/test01.txt"
    // "D:/Files/advent/2024/day03/data.txt"
    // "/Users/rocco/Programming/advent_zig/2024/day03/test01.txt"
    "/Users/rocco/Programming/advent_zig/2024/day03/data.txt"
); // zig fmt: on

fn loadData(allocator: Allocator, filename: []const u8) !AList([]const u8) {
    const file = try std.fs.cwd().openFile(filename, .{});
    defer file.close();
    const content = try file.readToEndAlloc(allocator, std.math.maxInt(u64));

    var data = AList([]const u8).init(allocator);
